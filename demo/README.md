...
这里收录线路工作台、SMT 对照和完整四逻辑 GHZ 回放。开发过程和失败尝试留在 `artifacts/`；运行 demo 不需要那个旧目录。
...
浏览器自动打开首页。保持终端运行，Ctrl+C 关闭三个本地服务。默认首页端口 8800 被占用时用 `python demo/launch.py --port 0`。两个编译服务总是自动分配端口，无需修改 HTML。无桌面环境时加 `--no-browser`，读取终端地址。
