...
from neutral_atom_experiments.surface_qec_temporal import HISTORY_IDS, CORRECTION_PREFIX, validate_history
...
def run_qec_temporal(state, *, on_event=None, **options):
    ...
    result=run_qec_joint(env if env is not None else state,on_event=observe,frontier_validator=frontier,**options)
