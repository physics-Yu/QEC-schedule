...
import uuid
from neutral_atom_app.visualization.workbench import validate_input, preview, compile_input, MAX_COMPILE_TIMEOUT_S
...
def create_server(port=8766,output='artifacts/workbench',timeout=90,restore_job_dir=None,ui_root=None):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path=='/': return self.respond(200,(root/('index.html' if ui_root else 'workbench.html')).read_text(encoding='utf-8'),'text/html; charset=utf-8')
            if path=='/workbench.js': return self.respond(200,(root/'workbench.js').read_text(encoding='utf-8'),'text/javascript; charset=utf-8')
            if path=='/studio-model.js': return self.respond(200,(root/('studio-model.js' if ui_root else 'studio_model.js')).read_text(encoding='utf-8'),'text/javascript; charset=utf-8')
            if path=='/atom-viewer.js': return self.respond(200,javascript(),'text/javascript; charset=utf-8')
            if path=='/studio-catalog.js':
                from neutral_atom_app.visualization.studio_config import catalog
                return self.respond(200,'window.AtomStudioCatalog='+canonical_json(catalog())+';', 'text/javascript; charset=utf-8')
            if path.startswith('/api/studio/demos/'):
                try:
                    from neutral_atom_app.visualization.studio_config import demo_input
                    return self.respond(200,demo_input(path.rsplit('/',1)[1]))
            ...
            if path=='/api/examples/surface-qec-temporal':
                try:
                    from neutral_atom_experiments.surface_qec_temporal import experiment_input
