...
def reduce_physical(state,event,queue):
    ...
    if event.event_type==EventType.PLAN_STARTED:
        ...
        plan=event.plan;gate=next(iter(plan.intent.gate_ids),None)
        runtime=PlanRuntime(plan,event.time_us)
        result=replace(state,dag=state.dag.transitioned(gate,GateStatus.RESERVED) if gate else state.dag,active_plan=runtime,
        ...
        return result,queue,{'reserved_resources':plan.resources,**task_metadata(plan)}
    ...
    plan=runtime.plan;gate=next(iter(plan.intent.gate_ids),None);metrics=state.physical_metrics
    ...
    if event.event_type==EventType.PLAN_COMPLETED:
        ...
        return replace(state,active_plan=None,reservations=(),physical_metrics=replace(metrics,
            ...
            completed_plan_count=metrics.completed_plan_count+1)),queue,extra|{'released_resources':plan.resources,**task_metadata(plan)}
    ...
    if event.event_type==EventType.OPERATION_STARTED:
        ...
        expected={OperationType.RAMAN_ROTATION:state.hardware.raman_duration_us,OperationType.TRAP_SWITCH:state.hardware.switch_duration_us,OperationType.AOD_LOAD:state.hardware.load_duration_us,OperationType.AOD_OFFLOAD:state.hardware.offload_duration_us,
        ...
        if kind==OperationType.AOD_MOVE:
            ...
            if kind in TRANSFERS:
                result=begin_transfer(backend,state,transfer_bindings,kind)
                extra={'transfer':result.transfer,'supports_before':trap_state(state),'supports_overlap':trap_state(result),
                       'switch_timing':'included_in_transfer_duration'}
            elif kind==OperationType.TRAP_SWITCH:
                switch_traps(state,operation.switch_state)
                extra={'supports_before':trap_state(state),'supports_target':operation.switch_state}
            elif kind==OperationType.RAMAN_ROTATION:
                rotation=validate_rotation(state,gate)
                result=replace(state,dag=state.dag.transitioned(gate,GateStatus.RUNNING))
                extra={'gate_id':gate,'input_gate':rotation,'u_parameters_rad':rotation.u_parameters,'resource':'RAMAN:'+rotation.qubit_ids[0]}
            else:
                pairs=backend.validate_pulse(state,gate)
                result=replace(state,dag=state.dag.transitioned(gate,GateStatus.RUNNING))
                extra={'actual_pairs':pairs,'intended_pairs':(tuple(sorted(state.dag.nodes[gate].gate.qubit_ids)),)}
        ...
        return result,queue,extra|{'operation_type':kind.value,'label':operation.label,'duration_us':duration,'transfer_bindings':operation.transfer_bindings,**task_metadata(plan,operation)}
    ...
    if kind==OperationType.AOD_LOAD:
        result=finish_transfer(backend,state,transfer_bindings,kind)
        metrics=replace(metrics,aod_load_count=metrics.aod_load_count+1,
            captured_atom_count_total=metrics.captured_atom_count_total+len(transfer_bindings),
            incidental_atom_transport_total=metrics.incidental_atom_transport_total+sum(b.atom_id in plan.incidental_atom_ids for b in transfer_bindings))
        extra={'captured_atom_ids':tuple(sorted(b.atom_id for b in transfer_bindings)),'incidental_atom_ids':tuple(sorted(b.atom_id for b in transfer_bindings if b.atom_id in plan.incidental_atom_ids))}
    elif kind in (OperationType.AOD_PARK,OperationType.AOD_RECAPTURE):
        recapture=kind==OperationType.AOD_RECAPTURE
        result=finish_transfer(backend,state,operation.transfer_bindings,kind)
        ...
        result=switch_traps(state,operation.switch_state)
    elif kind==OperationType.RAMAN_ROTATION:
        rotation=validate_rotation(state,gate)
        extra={'gate_id':gate,'input_gate':rotation,'u_parameters_rad':rotation.u_parameters,'effect_completed':True}
        result=replace(state,dag=state.dag.transitioned(gate,GateStatus.COMPLETED))
        metrics=replace(metrics,circuit_makespan_us=event.time_us,raman_busy_time_us=metrics.raman_busy_time_us+operation.duration_us)
    else:
        extra={'actual_pairs':backend.validate_pulse(state,gate),'gate_id':gate,'effect_completed':True}
        result=replace(state,dag=state.dag.transitioned(gate,GateStatus.COMPLETED))
    ...
    if kind!=OperationType.RAMAN_ROTATION:
    ...
    return result,queue,extra|{'operation_type':kind.value,'label':operation.label,'supports_after':trap_state(result),
                              'transfer_stage':'committed' if kind in TRANSFERS else None,**task_metadata(plan,operation)}


def task_metadata(plan,operation=None):
    if not isinstance(plan.intent,TaskIntent):
        return {}
    value={'task_id':plan.intent.task_id,'task_phase':plan.intent.phase,
           'related_gate_id':plan.intent.related_gate_id}
    if operation is not None:
        value.update(operation_ref=f'{plan.intent.task_id}/{operation.id}',effect_gate_id=operation.gate_id)
    return value
