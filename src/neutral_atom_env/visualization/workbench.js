...
const mixed=()=>({compiler:'greedy',ez_policy:'adaptive',max_decisions:10000,atom_count:4,layout:'row',seed:7,anchor_order:'forward',gates:[
 {id:'G000',gate_type:'H',qubit_ids:['Q000'],parameters:[],column:0},
 {id:'G001',gate_type:'CZ',qubit_ids:['Q000','Q001'],parameters:[],column:1},
 {id:'G002',gate_type:'T',qubit_ids:['Q000'],parameters:[],column:2},
 {id:'G003',gate_type:'H',qubit_ids:['Q002'],parameters:[],column:0},
 {id:'G004',gate_type:'T',qubit_ids:['Q002'],parameters:[],column:1},
 {id:'G005',gate_type:'T',qubit_ids:['Q003'],parameters:[],column:0}]});
const m4compare=()=>({...mixed(),compiler:'lookahead',site_limit:1,beam_width:2,rollout_budget:6,gates:[{id:'G000',gate_type:'CZ',qubit_ids:[qid(0),qid(1)],parameters:[],column:0},{id:'G001',gate_type:'CZ',qubit_ids:[qid(0),qid(1)],parameters:[],column:1},{id:'G002',gate_type:'T',qubit_ids:[qid(0)],parameters:[],column:2},{id:'G003',gate_type:'H',qubit_ids:[qid(2)],parameters:[],column:0}]});
const multitrap=()=>({...mixed(),aod_traps:4,gates:[{id:'G000',gate_type:'CZ',qubit_ids:[qid(0),qid(1)],parameters:[],column:0},{id:'G001',gate_type:'CZ',qubit_ids:[qid(2),qid(3)],parameters:[],column:0},{id:'G002',gate_type:'T',qubit_ids:[qid(0)],parameters:[],column:1},{id:'G003',gate_type:'T',qubit_ids:[qid(2)],parameters:[],column:1}]});
const aodraman=()=>({...mixed(),gates:[
 {id:'G000',gate_type:'CZ',qubit_ids:[qid(0),qid(1)],parameters:[],column:0},
 {id:'G001',gate_type:'T',qubit_ids:[qid(0)],parameters:[],column:1},
 {id:'G002',gate_type:'T',qubit_ids:[qid(1)],parameters:[],column:1},
 {id:'G003',gate_type:'CZ',qubit_ids:[qid(0),qid(1)],parameters:[],column:2}]});
const parallel1q=()=>({...mixed(),gates:['H','H','H','H'].map((kind,i)=>({id:'G00'+i,gate_type:kind,qubit_ids:[qid(i)],parameters:[],column:0}))});
const parallel=()=>({...mixed(),gates:[
 {id:'G000',gate_type:'CZ',qubit_ids:['Q000','Q001'],parameters:[],column:0},
 {id:'G001',gate_type:'H',qubit_ids:['Q002'],parameters:[],column:0},
 {id:'G002',gate_type:'X',qubit_ids:['Q002'],parameters:[],column:1},
 {id:'G003',gate_type:'T',qubit_ids:['Q002'],parameters:[],column:2},
 {id:'G004',gate_type:'H',qubit_ids:['Q003'],parameters:[],column:0},
 {id:'G005',gate_type:'CZ',qubit_ids:['Q000','Q001'],parameters:[],column:1},
 {id:'G006',gate_type:'CZ',qubit_ids:['Q000','Q001'],parameters:[],column:2},
 {id:'G007',gate_type:'T',qubit_ids:['Q000'],parameters:[],column:3}]});
let draft=parallel(),revision=0,columns=8,page=0,tool='CZ',selected=null,pending=null,history=[],future=[];
...
function adoptTemplate(value){const compilation=clone(draft.compilation);draft=normalizeConfiguration(value);draft.compilation=compilation;syncCompilation(draft);columns=8;page=0;}
...
const compilationPresets=[
 {id:'greedy',label:'贪心并行与运输',group:'M4 调度',family:'physical',defaults:{ready_limit:16,site_limit:4}},
 {id:'critical_path',label:'关键路径优先',group:'M4 调度',family:'physical',defaults:{ready_limit:16,site_limit:4}},
 {id:'lookahead',label:'有限前瞻与复用',group:'M4 调度',family:'physical',defaults:{ready_limit:16,site_limit:4,lookahead_depth:2,beam_width:3,rollout_budget:12}},
 {id:'basic',label:'逐门归还基线',group:'M4 调度',family:'physical',defaults:{ready_limit:16,site_limit:4}},
 {id:'patch_greedy',label:'二维 patch 并行贪心',group:'二维批量运输',family:'patch',defaults:{row_candidate_budget:4096,route_expansions:100000}},
 {id:'patch_symmetric',label:'二维 patch 对称基线',group:'二维批量运输',family:'patch',defaults:{row_candidate_budget:4096,route_expansions:100000}},
 {id:'qec_joint',label:'批光与读出联合优化',group:'QEC 单轮调度',family:'qec',defaults:{row_candidate_budget:4096,route_expansions:100000}},
 {id:'qec_persistent',label:'保留 AOD 状态与复用',group:'QEC 单轮调度',family:'qec',defaults:{row_candidate_budget:4096,route_expansions:100000}},
 {id:'qec_ghz2',label:'测量与条件纠错基线',group:'QEC 单轮调度',family:'qec',defaults:{row_candidate_budget:4096,route_expansions:100000}}
];
function presetUnavailable(p){
 if(p.family==='qec')return draft.circuit_profile==='qec_ghz2'?'':'需单轮 QEC';
 if(draft.circuit_profile!=='physical')return '需普通物理线路';
 return p.family==='patch'&&draft.layout!=='surface_patches'?'需二维 patch 布局':'';
}
function renderConfiguration(){
 const c=draft.compilation,temporal=['qec_temporal','qec_temporal_four'].includes(draft.circuit_profile);
 const currentPreset=c.strategy==='legacy'?compilationPresets.find(p=>p.id===c.implementation):null;
 const groups=[...new Set(compilationPresets.map(p=>p.group))];
 $('compiler').innerHTML='<optgroup label="默认配置"><option value="recommended">推荐调度'+(temporal?' · 多轮 QEC 联合优化':'')+'</option><option value="baseline" '+(temporal?'disabled':'')+'>基线对照'+(temporal?'（当前多轮协议未支持）':'')+'</option></optgroup>'+groups.map(group=>'<optgroup label="'+group+'">'+compilationPresets.filter(p=>p.group===group).map(p=>{const reason=presetUnavailable(p);return '<option value="preset:'+p.id+'" '+(reason?'disabled':'')+'>'+p.label+(reason?'（'+reason+'）':'')+'</option>';}).join('')+'</optgroup>').join('')+(c.strategy==='legacy'&&!currentPreset?'<option value="legacy">历史配置 · '+escape(c.implementation)+'</option>':'');
 $('compiler').value=currentPreset?'preset:'+currentPreset.id:c.strategy;
 $('strategy-preset-note').textContent='内置预设带搜索默认值，选用时保留编译时限。可在高级预算中调整，再保存为自己的配置。灰色选项标明所需协议或布局。';
 $('policy-note').textContent=policyNotes[draft.compiler]||'历史配置按原实现执行。';
 ...
 $('open-compilation').textContent='编译 · '+(currentPreset?.label||strategyNames[c.strategy]);
 $('legacy-controls').hidden=c.strategy!=='legacy'||Boolean(currentPreset);
 $('compilation-compatibility').textContent=temporal?'当前多轮协议已接入带历史校验的联合调度；尚无已支持的多轮基线。线路内容和协议不会随调度设置更换。':'同一编译配置可应用于不同线路。推荐调度按已有平台能力选择实现；基线用于同条件对照。';
}
$('compiler').onchange=()=>{
 const strategy=$('compiler').value;
 if(strategy.startsWith('preset:')){
  const preset=compilationPresets.find(p=>'preset:'+p.id===strategy);
  if(!preset||presetUnavailable(preset)){render();toast('该预设不适用于当前协议或布局；线路已保留。');return;}
  change(()=>{const timeout=draft.compilation.compile_timeout_s;draft.compilation={strategy:'legacy',implementation:preset.id,max_decisions:10000,...preset.defaults};if(timeout!==undefined)draft.compilation.compile_timeout_s=timeout;});return;
 }
 change(()=>{draft.compilation.strategy=strategy;delete draft.compilation.implementation;});
};
...
function change(mutator){history.push(clone(draft));if(history.length>60)history.shift();future=[];mutator();selected=null;pending=null;edited()}
function edited(){revision++;$('compile').disabled=false;syncCompilation(draft);toast();$('diagnostics').hidden=true;$('revision').textContent='DRAFT '+revision;render();markStale();
...
async function updatePreview(rev){if(previewController)previewController.abort();previewController=new AbortController();try{
 ...
 scene.traps.map(t=>`<circle cx="${X(t.position.x_um)}" cy="${Y(t.position.y_um)}" r="${t.enabled===false?2:3.8}" fill="none" stroke="${t.enabled===false?'#c6cfc9':'#93a3b0'}"><title>${t.id} ${t.enabled===false?'关闭的候选位置':'初始开启'}</title></circle>`).join('')+
 ...
 }catch(e){if(e.name!=='AbortError'&&rev===revision){toast(e.message);$('layout-caption').textContent='当前输入无有效预览';}}
}
function render(){
 $('qec-enabled').checked=Boolean(draft.qec_enabled);$('qec-options').hidden=!draft.qec_enabled;
 const origins=draft.qec_patch_origins||defaultOrigins();
 for(let i=0;i<4;i++){if(i>=2)$('qec-patch-control-'+i).hidden=!isFourPatch();$('qec-patch-origin-'+i).value=(origins[i]||[0,0]).join(', ');}
 $('qec-profile-note').textContent=(isFourPatch()?'36 data + 32 ancilla；四块 patch 使用独立扩展的 SZ 平台。':'18 data + 16 ancilla；')+'本模式支持 H/X/Y/Z/CZ/MEASURE/RESET。T 超出 Clifford tableau 模型。测量 500 μs、复位 100 μs 是本次仿真假设。';
 const faultGate=draft.gates.find(g=>g.id==='QEC_FAULT');$('qec-fault').value=faultGate?.gate_type||'none';
 $('qec-fault-target').innerHTML=Array.from({length:draft.qec_enabled?qecDataCount():0},(_,i)=>`<option ${faultGate?.qubit_ids[0]===qid(i)?'selected':''}>${qid(i)}</option>`).join('');
 $('qec-fault-target').disabled=!draft.qec_enabled||isTemporal();$('qec-fault').disabled=isTemporal();
 $('qec-fault-note').textContent=isTemporal()?'多轮协议：报告翻转请编辑对应 MEASURE 门；数据故障门与轮边界见 JSON。旧单轮故障控件已禁用，避免覆盖现有线路。':'修改故障仅替换协议中的故障门，不重建已编辑线路。';
 $('qec-roles').textContent=draft.qec_enabled?(isFourPatch()?'Q000–Q035: data · Q036–Q067: ancilla':'Q000–Q017: data · Q018–Q033: ancilla'):'';
 ...
 $('atom-count').value=draft.atom_count;$('layout').value=draft.layout;$('seed').value=draft.seed;$('anchor-order').value=draft.anchor_order;renderConfiguration();
 ...
 $('policy-note').textContent=policyNotes[draft.compiler]||'M3 历史对照策略。';
...
$('preset').onchange=()=>{const value=$('preset').value;if(!value)return;const compilation=clone(draft.compilation);change(()=>{draft=value==='m4compare'?m4compare():value==='multitrap'?multitrap():value==='aodraman'?aodraman():value==='parallel'?parallel():value==='parallel1q'?parallel1q():mixed();if(value==='empty')draft.gates=[];if(value==='rotations')draft.gates=[{id:'G000',gate_type:'T',qubit_ids:['Q000'],parameters:[],column:0},{id:'G001',gate_type:'T',qubit_ids:['Q001'],parameters:[],column:1}];if(value==='chain')draft.gates=[draft.gates[0],...[[0,1],[1,2],[2,3]].map(([a,b],i)=>({id:'G00'+(i+1),gate_type:'CZ',qubit_ids:[qid(a),qid(b)],parameters:[],column:i+1}))];draft=normalizeConfiguration(draft);draft.compilation=compilation;});$('preset').value='';};
$('surface-ghz').onclick=async()=>{const rev=revision;try{const value=await api('/api/examples/surface-ghz');if(rev!==revision)throw Error('载入期间草稿已变化，请再次载入。');change(()=>{adoptTemplate(value);});}catch(e){toast('示例载入失败：'+e.message);}};
async function loadQecExample(compiler='qec_ghz2'){const rev=revision;try{const value=await api('/api/examples/'+(compiler==='qec_temporal_four'?'surface-qec-temporal-four':compiler==='qec_temporal'?'surface-qec-temporal':'surface-qec-ghz2'));if(typeof compiler==='string'&&['qec_ghz2','qec_persistent','qec_joint','qec_temporal','qec_temporal_four'].includes(compiler))value.compiler=compiler;if(rev!==revision)throw Error('载入期间草稿已变化，请再次载入。');change(()=>{adoptTemplate(value);});}catch(e){render();toast('QEC 示例载入失败：'+e.message);}}
for(let i=0;i<4;i++)$('qec-patch-origin-'+i).onchange=()=>{
 const parts=$('qec-patch-origin-'+i).value.split(',').map(x=>x.trim());
 const point=parts.map(Number);
 if(!draft.qec_enabled||i>=defaultOrigins().length||parts.length!==2||parts.some(x=>x==='')||point.some(x=>!Number.isFinite(x)||!Number.isInteger(x/5))){render();toast('Patch 原点须填写两个有限的 5 μm 格点坐标：x, y。');return;}
 change(()=>{const origins=clone(draft.qec_patch_origins||defaultOrigins());origins[i]=point;draft.qec_patch_origins=origins;});
};
$('surface-qec-ghz2').onclick=loadQecExample;
$('surface-qec-temporal').onclick=()=>loadQecExample('qec_temporal');
$('surface-qec-temporal-four').onclick=()=>loadQecExample('qec_temporal_four');
function changeQecFault(){
 if(isTemporal()){render();toast('多轮协议请直接编辑测量或数据故障门；不使用单轮故障控件。');return;}
 const kind=$('qec-fault').value,target=$('qec-fault-target').value,meta=draft.qec_protocol;
 if(!draft.qec_enabled||!meta||!Number.isInteger(meta.fault_column)){render();toast('当前线路没有故障阶段元数据；请直接在门编辑器修改。');return;}
 const id=meta.fault_gate_id||'QEC_FAULT',old=draft.gates.find(g=>g.id===id);
 const finalIds=new Set((meta.stages||[]).filter(s=>s.stage==='final-X-checks').map(s=>s.gate_id));
 const previous=old?.depends_on||[...new Set(draft.gates.filter(g=>finalIds.has(g.id)).flatMap(g=>g.depends_on||[]))];
 if(kind!=='none'&&draft.gates.some(g=>g.id!==id&&g.column===meta.fault_column&&g.qubit_ids.includes(target))){render();toast('故障列目标已有门，请移动该门或直接编辑故障门。');return;}
 change(()=>{
  draft.gates=draft.gates.filter(g=>g.id!==id).map(g=>{const refs=g.depends_on||[];if(refs.includes(id))return {...g,depends_on:[...new Set(refs.flatMap(ref=>ref===id?previous:[ref]))]};return g;});
  if(kind!=='none'){draft.gates.push({id,gate_type:kind,qubit_ids:[target],parameters:[],column:meta.fault_column,depends_on:previous});draft.gates=draft.gates.map(g=>finalIds.has(g.id)?{...g,depends_on:[...new Set([...(g.depends_on||[]),id])]}:g);}
  draft.qec_fault=kind==='none'?null:{pauli:kind,qubit_id:target};
 });
}
$('qec-fault').onchange=changeQecFault;$('qec-fault-target').onchange=changeQecFault;
...
async function compile(){if(job||$('compile').disabled)return;$('compile').disabled=true;const rev=revision,value=clone(syncCompilation(draft));$('diagnostics').hidden=true;toast();status('compiling','正在编译当前线路',`版本 ${rev} · 准备物理输入…`);$('cancel').disabled=false;
...
$('import-file').onchange=async()=>{const file=$('import-file').files[0];if(!file)return;const rev=revision;try{if(file.size>2*1024*1024)throw Error('线路文件不能超过 2 MiB');const parsed=JSON.parse(await file.text()),valid=await api('/api/preview',parsed);if(rev!==revision)throw Error('导入期间草稿已变化，请再次导入。');change(()=>{draft=normalizeConfiguration(valid.input);columns=8;page=0;});}catch(e){toast('导入失败：'+e.message);}finally{$('import-file').value='';}};
...
async function openInitialLink(){
 ...
 try{
  ...
  if(id){
   ...
   if(!['surface-ghz','surface-qec-ghz2','surface-qec-temporal','surface-qec-temporal-four'].includes(example))throw Error('未知示例。');
   value=await api('/api/examples/'+example);
  }
  if(rev!==revision)return;
  change(()=>{draft=normalizeConfiguration(value);columns=8;page=0;});
  if(next)acceptResult(next,revision,value,id);
  else status('idle','实验线路已载入',value.atom_count+' 原子实验草稿可编辑；点击编译开始真实物理执行。');
