...
class MotionCompiler:
    ...
    def compile(self,intent,state,*,target_configuration=None):
        ...
        if state.hardware.backend=='rigid' and state.hardware.selective_transfer_enabled and not anchors:
            ...
            from neutral_atom_strategies.motion.rigid_parking_compiler import compile_parked_pair
        ...
        if state.hardware.backend=='row_column' and len(anchors)!=1:
            from neutral_atom_strategies.motion.row_column_compiler import compile_mobile_pair
        ...
        from neutral_atom_strategies.motion.routing import plan_transport
        if target_configuration is not None:
