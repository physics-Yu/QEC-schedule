...
| `architecture` / `coordinate_mode` | 当前默认 row_column_orthogonal / relative_offsets 的声明；加载器拒绝虚构能力。 |
...
模式边界保持：完整 demo 配套锁定、自定义独立选算法、电路示例仅替换门、手动编译。当前有序工作台支持二维非均匀轴；旧 M4 的刚性单行限制仅适用于历史兼容输入。详细归属与路线见 [架构说明](../../docs/studio_configuration_layers.md)。
