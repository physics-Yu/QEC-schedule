"""One declared step-2 attempt. Failure is reported, never automatically retried."""
import argparse
...
def main():
    ...
    parser=argparse.ArgumentParser();parser.add_argument('--output',default='artifacts/qec-roadmap/step2-attempt1')
    ...
    (output/'strategy.json').write_text(canonical_json({'actual_strategy':'qec_persistent','reference_strategy':value['compiler'],
    ...
    try:
        result=run_qec_persistent(state,on_event=observe,terminal=terminal)
        ...
        assert state.time_us<contract['requirements']['step2_total_time_strictly_below_us'],'NO_TIME_IMPROVEMENT'
        assert state.metrics()['aod_load_count']<contract['requirements']['step2_loads_strictly_below'],'NO_LOAD_IMPROVEMENT'
    ...
    for name,data in [('recording',payload),('qec_result',summarize(state)),('result',{'status':'failed' if error else 'checks_passed_replay_pending',
            'actual_strategy':'qec_persistent','metrics':state.metrics(),'compile_seconds':elapsed,'error':error}),
    ...
    if error:
        failure={'facts':[f"步骤2首次正式完整对照未通过：{error['message']}",f"实际完成{state.metrics()['completed_gate_count']}/{len(circuit.gates)}，物理时间{state.time_us} μs"],
            'hypotheses':['原因待只读诊断；不得把配对分支不匹配误报为物理无解。'],
            'proposed_retry_scope':['先定位失败记录，明确拟议修正和验证范围，用户在对话审批后才启动新尝试。']}
        update('artifacts/qec-roadmap/status.json','step2','fail',failure=failure,evidence=[{'label':'实际失败产物','path':'step2-attempt1/result.json'}])
