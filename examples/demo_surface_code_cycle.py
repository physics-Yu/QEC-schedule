...
def main():
    ...
    parser.add_argument('--distance', type=int, default=3, help='Odd rotated-surface-code distance')
    ...
    try:
        code = create_code(distance=args.distance)
        trace, result = run_cycle(load_hardware_config(args.config), code=code,
    ...
    print(f"Completed rotated_surface d={args.distance}: {result['physical_gate_count']} gates / "
          f"{result['epoch_count']} physical epochs in {trace['duration']:.3f} us")
