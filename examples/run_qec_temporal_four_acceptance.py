...
def check_final(state,circuit,terminal):
    ...
    assert report['verified_logical_ghz4'] and report['measurement_protocol_complete'],report
    assert validate_history(state.measurement_results)['supported']
    assert len(state.measurement_results)==160
...
def compile_case(output,attempt=1):
    ...
    dump(output/'strategy.json',{'actual_strategy':'qec_temporal_four','stage':'4C','attempt':attempt})
    ...
    def observe(s,event):
        ...
        progress={'completed_gates':count,'total_gates':len(circuit.gates),'simulation_time_us':s.time_us,
                  'elapsed_seconds':perf_counter()-started,'trace_records':len(s.trace.records)}
    ...
    try:
        result=run_qec_temporal_four(state,on_event=observe,terminal=terminal)
        assert result.status=='completed',str(result.diagnostics)
        check_final(state,circuit,terminal)
        assert len(state.trace.records)<8192,'Representative exceeded the declared parsed-event cache working set'
    ...
    dump(output/'result.json',{'status':'failed' if error else 'checks_passed_replay_pending',
        'actual_strategy':'qec_temporal_four','error':error,'compile_seconds':perf_counter()-started,'metrics':state.metrics(),
        'trace_records':len(state.trace.records)})
...
def verify_case(output):
    ...
    for line in (output/'trace.jsonl').open(encoding='utf-8'):
        ...
        if plans%20==0:
            progress={'replayed_plans':plans,'completed_gates':state.metrics()['completed_gate_count'], 'elapsed_seconds':perf_counter()-started}
...
def main(args):
    ...
    if args.mode=='compile':return compile_case(output,args.attempt)
    if args.mode=='verify':return verify_case(output)
    output.mkdir(parents=True,exist_ok=False)
    dump(output/'attempt.json',{'stage':'4C','attempt':args.attempt,'started_at':datetime.now(timezone.utc).isoformat(),
        'compile_budget_seconds':3600,'verify_budget_seconds':3600,
        'contract':f'docs/qec_temporal_four_attempt{args.attempt}.md'})
    for mode in ('compile','verify'):
        ...
        try:
            with (output/(mode+'-output.txt')).open('w',encoding='utf-8') as log:
                run=subprocess.run([sys.executable,__file__,'--mode',mode,'--output',str(output),'--attempt',str(args.attempt)],
                    cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,timeout=3600)
                code=run.returncode
        except subprocess.TimeoutExpired:
            with (output/(mode+'-output.txt')).open('a',encoding='utf-8') as log:
                log.write('DECLARED_3600_SECOND_BUDGET_EXCEEDED\n')
            code=124
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--output',default='artifacts/qec-roadmap/step4C-attempt1')
