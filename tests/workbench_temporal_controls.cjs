...
const example={compiler:'qec_temporal',qec_enabled:true,layout:'surface_qec_ghz2',atom_count:34,
 ...
 qec_patch_origins:[[0,0],[40,0]],ez_policy:'adaptive',gates:[
 {id:'m',gate_type:'MEASURE',qubit_ids:['Q018'],parameters:[],column:0,readout_flip:true},
...
const sandbox={console,AbortController,Blob,setTimeout,clearTimeout,
 ...
 fetch:async(path,options)=>{
  ...
  else if(path==='/api/examples/surface-qec-temporal'||path==='/api/examples/surface-qec-ghz2')data=JSON.parse(JSON.stringify(example));
  ...
  else if(path==='/api/jobs/large/result')data={status:'completed',input:compiled,recording:recording(compiled.atom_count),compile_seconds:0,decision_log:[],qec_result:{verified_logical_ghz2:false,measurement_protocol_complete:false,history_complete:false,history_supported:false,logical_xx:0,logical_zz:0,true_measurement_results:{m:0},reported_measurement_results:{m:1},corrections:[],decoded_corrections:[{gate_type:'X',qubit_ids:['Q001']}]}};
...
async function main(){
 ...
 await el('surface-qec-temporal').onclick();
 const original=await exported();assert.equal(original.compiler,'qec_temporal');
 ...
 console.log('PASS temporal editor flags, undo, JSON, original gates/axes, legacy guard and true/reported/unsupported display (UI doubles only)');
}
