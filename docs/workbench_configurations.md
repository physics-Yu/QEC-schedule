...
**2026-09-14 更新：** 当前入口与配置归属以 [工作台分层规范](studio_configuration_layers.md) 为准。顶部完整 demo 锁定配套初态/平台/线路/算法；自定义仅提供六种通用策略，纯电路示例只换门列表。下面的“模板同时带平台、推荐按 GHZ 切换专用实现、九个预设含 patch/QEC”是旧版行为，已从新 UI 移除，后台历史输入兼容保留。AOD 是相对偏移；多交点 M4 规划器现仅支持 1×N、10 μm 等间距，界面与 API 在编译前检查，不能把硬件支持非均匀矩形等同于通用规划已接入。
